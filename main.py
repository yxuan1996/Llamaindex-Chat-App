...
# Initialize chat store (we'll use SimpleChatStore in memory for simplicity)
# In production, you'd want to persist this to Supabase
...
def get_chat_engine(user_id: str, thread_id: str = "default"):
    ...
    if key not in user_chat_engines:
        # Create memory buffer for this thread
...
async def get_threads(user = Depends(get_current_user)):
    ...
    Returns available thread IDs
    """
    user_id = user.user.id
    
    # Find all threads for this user
    threads = [key.split("_", 1)[1] for key in user_chat_engines.keys() 
               if key.startswith(f"{user_id}_")]
    
    return {"threads": threads if threads else ["default"]}
...
async def delete_thread(thread_id: str, user = Depends(get_current_user)):
    ...
    Removes the chat history and engine for that thread
    """
    user_id = user.user.id
    key = f"{user_id}_{thread_id}"
